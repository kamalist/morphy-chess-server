...
public class RemoveListCommand extends AbstractCommand {
	...
	public void process(String arguments, UserSession userSession) {
		...
		if (value.equals("*")) {
			myList.clear();
			userSession.send("All players have been removed from your " + listName + " list.");
			return;
		}
		...
		if (myList.contains(value)) {
			if (list == PersonalList.channel) { 
				...
				} catch(NumberFormatException e) { userSession.send("The channel to remove must be a number between " + Channel.MINIMUM + " and " + Channel.MAXIMUM + "."); return; }
			}
			
			myList.remove(value);
			userSession.send("[" + value + "] removed from your " + listName + " list.");
		} else {
			userSession.send("[" + value + "] is not in your " + listName + " list.");
		}
	}
}
