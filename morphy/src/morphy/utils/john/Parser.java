...
public class Parser {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < list.length; i++) {
			String cmd = ("" + list[i].name().charAt(0)).toUpperCase()
					+ list[i].name().substring(1);
			String myStr = str.replaceAll("AddChannel", "Add" + cmd);
			myStr = myStr.replaceAll("PersonalList.channel", "PersonalList."
					+ list[i].name());
			myStr = myStr.replaceAll(" channel ", " " + list[i].name() + " ");
			FileWriter writer = new FileWriter(new File(
					"C:\\Users\\John\\Desktop\\Add" + cmd + "Command.java"));
			writer.write(myStr);
			writer.close();
		}
	}
